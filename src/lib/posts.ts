...
export function formatContentToHtml(rawContent: string): string {
  ...
  html = html.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (_match, alt, url) => {
    return `<img src="${url}" alt="${alt || 'OSSEGREDOSDOVITORIO'}" class="my-6 rounded-2xl w-full max-h-[500px] object-cover shadow-md" />`;
  });
...
const DOMAIN = 'ossegredosdovitorio.com.br';
